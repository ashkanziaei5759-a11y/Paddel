...
interface ImageSliderProps extends React.HTMLAttributes<HTMLDivElement> {
  images: string[];
  /**
   * نسخه‌ی افقیِ هر تصویر برای نمایشگر بزرگ. مرورگر خودش بر اساس عرض صفحه
   * یکی را برمی‌دارد، پس روی گوشی فایل افقی اصلاً دانلود نمی‌شود.
   */
  wideImages?: string[];
  interval?: number;
  /**
   * کلاس‌های اضافی روی خودِ تصویر — برای جای سوژه در کادر وقتی بریده می‌شود.
   * کلاس می‌گیریم نه style، چون style درون‌خطی بر واریانت‌های واکنش‌گرا
   * (مثل lg:) غلبه می‌کند و آن‌ها بی‌اثر می‌شوند.
   */
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "relative h-full w-full overflow-hidden bg-brand-900",
          className,
        )}
      ...
      >
        <AnimatePresence initial={false}>
          ...
          >
            <picture>
              {wideImages?.[currentIndex] && (
                <source
                  media="(min-width: 1024px)"
                  srcSet={wideImages[currentIndex]}
                />
              )}
              ...
              <img
                ...
                alt={
                  failed[currentIndex]
                    ? ""
                    : (alts?.[currentIndex] ?? `تصویر ${currentIndex + 1}`)
                }
                className={cn(
