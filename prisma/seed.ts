 ...
 *   · پله‌های جریمه‌ی لغو
 *   · چند بازیکن نمونه و یک تورنومنت آماده‌ی ثبت‌نام
 *
...
async function main() {
  ...
  console.info(`✅ ${COURTS.length} زمین با قیمت‌گذاری ساعات پرتقاضا آماده شد`);
  ...
  console.info(`✅ ${players.length} بازیکن نمونه ساخته شد (رمز عبور: Player@12345)`);
  ...
  if (!existingTournament) {
    ...
    console.info(`✅ تورنومنت نمونه ساخته شد: ${tournament.name}`);
  }
